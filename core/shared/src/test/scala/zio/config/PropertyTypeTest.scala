...
object PropertyTypeTestUtils {
  ...
  private def genOptionalStr(gen: Gen[Sized, String]) =
    oneOf(const(""), gen)

  private val genDigit0To9: Gen[Any, String] = char('0', '9').map(_.toString)
  private val genDigit1To9: Gen[Any, String] = char('1', '9').map(_.toString)

  /**
   * Generates a string of up to n digits. Default maxLength digits is 100.
   */
  private def genDigits(maxLength: Int = 100): Gen[Sized, String] =
    sized(n => listOfN(math.min(n, maxLength))(genDigit0To9).map(_.mkString))

  val genValidBigIntString: Gen[Sized, String] = for {
  ...
  private def genAppend(
    ...
    for {
      str  <- as.headOption.fold[Gen[Sized, String]](const(""))(identity)
      rest <- if (as.isEmpty) const("") else genAppend(as.tail: _*)
    } yield str + rest

  private val leadingZeros: Gen[Sized, String] =
    listOf(const('0')).map(_.mkString)

  def genPadLeadingZeros[A: Numeric](gen: Gen[Any, A]): Gen[Sized, String] =
    (leadingZeros <*> gen.map(_.toString)).map { case (zeros, num) =>
      if (num.startsWith("-")) "-" + zeros + num.drop(1)
      else zeros + num
    }

  private val genWhole: Gen[Sized, String] = oneOf(
    const("0"),
    genAppend(genDigit1To9, genDigits())
  )

  private val genDecimal: Gen[Sized, String] = oneOf(
    genOptionalStr(const(".")),
    genAppend(const("."), genDigits())
  )

  private val genExponent: Gen[Sized, String] = genAppend(
  ...
  val genValidBigDecimalString: Gen[Sized, String] = genAppend(
    genOptionalStr(const("-")),
  ...
  val genAlphaChar: Gen[Any, Char] =
    oneOf(char(65, 90), char(97, 122))

  private val genScheme: Gen[Sized, String] = for {
  ...
  private val genAuth: Gen[Sized, String] = genAppend(
    const("//"),
  ...
  private val genPath: Gen[Sized, String]  =
    listOf1(listOf1(alphaNumericChar).map(_.mkString)).map(_.mkString("/"))

  private val genAuthorityAndPath: Gen[Sized, String] = {
    for {
  ...
  private val genQuery: Gen[Sized, String] = genAppend(
    const("?"),
    oneOf(alphaNumericString, listOfN(2)(alphaNumericString).map(_.mkString("=")))
  )

  private val genFragment: Gen[Sized, String] =
    alphaNumericString
   ...
   */
  val genValidUriString: Gen[Sized, String] = genAppend(
    genScheme,
    const(":"),
    genAuthorityAndPath,
  ...
  val genValidUrlString: Gen[Sized, String] = genAppend(
