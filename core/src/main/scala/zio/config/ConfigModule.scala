...
trait ConfigModule { module =>
  ...
  sealed trait ConfigDescriptor[A] { self =>
    ...
    final def from(that: ConfigSource[K, V]): ConfigDescriptor[A] =
    ...
    final def updateSource(f: ConfigSource[K, V] => ConfigSource[K, V]): ConfigDescriptor[A] = {
      def loop[B](config: ConfigDescriptor[B]): ConfigDescriptor[B] = config match {
  ...
  case class DynamicMap[A](source: ConfigSource[K, V], config: ConfigDescriptor[A]) extends ConfigDescriptor[Map[K, A]]
  ...
  case class Sequence[A](source: ConfigSource[K, V], config: ConfigDescriptor[A]) extends ConfigDescriptor[List[A]]

  case class Source[A](source: ConfigSource[K, V], propertyType: PropertyType[V, A]) extends ConfigDescriptor[A]
  ...
  def listStrict[A](desc: ConfigDescriptor[A]): ConfigDescriptor[List[A]] =
    Sequence(ConfigSource.empty, desc)
  ...
  def mapStrict[A](desc: ConfigDescriptor[A]): ConfigDescriptor[Map[K, A]] =
    DynamicMap(ConfigSource.empty, desc)
