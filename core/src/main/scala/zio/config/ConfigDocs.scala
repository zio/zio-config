...
object ConfigDocs {
  ...
  final case class DynamicMapInit[K, V](element: ConfigDocs[K, V])              extends ConfigDocs[K, V]
  // During reporting, we know the dynamic keys; Fixme; Make it better orthogonal
  final case class DynamicMap[K, V](element: Map[K, ConfigDocs[K, V]]) extends ConfigDocs[K, V]
