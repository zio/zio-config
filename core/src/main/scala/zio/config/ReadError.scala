...
object ReadError {
  final case class MissingValue[K](key: K, positions: List[Int] = Nil)    extends ReadError[K, Nothing]
   ...
   * @param positions For some key, there will be values supposed to be existing in multiple positions in the config source. The starting pos is 0.
   * @tparam K
   */
  def missingValue[K, V](key: K, positions: List[Int] = Nil): ReadError[K, V] =
    MissingValue(key)
