...
object ConfigSource {
  ...
  def mk[K, V](tree: PropertyTree[K, V],
               descriptions: Set[String]): ConfigSource[K, V] = {
  ...
  def fromSystemEnv(
    ...
    UIO
      ...
      .map(
        map =>
          ConfigSource
            .fromMap(map, SystemEnvironment, keyDelimiter, valueDelimiter)
      )
  ...
  private def unwrapSingletonLists[K, V](
  ...
  private[config] def fromPropertyTree[K, B](
                ...
                case (This(l1), This(l2)) =>
                  ...
                  keysAndTrees match {
                    case Some((index, trees)) =>
                      ...
                      keys.fold(List.empty[PropertyTree[String, String]]) {
                        nestedKeys =>
                          trees
                            .map(
                              tree =>
  ...
  private[config] def mergeAll[K, V](
    sources: Iterable[ConfigSource[K, V]]
  ): ConfigSource[K, V] =
