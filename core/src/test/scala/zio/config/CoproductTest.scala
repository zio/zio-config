...
//            assert(readLeft(p), isLeft(equalTo(EnterpriseAuth(Ldap(p.vLdap), DbUrl(p.vDbUrl)))))
//          }
//        },
//        testM("right element satisfied") {
//          check(genTestParams) { p =>
//            assert(readRight(p), isRight(equalTo(PasswordAuth(p.vUser, p.vCount, p.vFactor, Duration(p.vCodeValid)))))
//          }
//        },
...
//                  FormatError(Vector(p.kFactor), ReadFunctions.parseErrorMessage("notafloat", "float"))
//                )
//              )
//
