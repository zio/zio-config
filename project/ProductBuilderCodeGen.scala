...
object ProductBuilderCodeGen {
  ...
  val createProductBuilder = taskKey[Unit]("Run the entire stack of integration test.")

  createProductBuilder := {
    val file = sourceDirectory.value  / "main" / "scala" / "zio" / "config" / "ProductBuilder.scala"
    replaceFileSection(
      file,
      "productbuilder",
      productBuilderCodes :+ ""
  ...
  // shabby-code: ON
    ...
    def replaceFileSection(filepath: File, marker: String, newContents: List[String]): Unit = {
      ...
      println(filepath)
      println(toWrite.mkString("\n"))
      // IO.write(file / "ProductBuilder.scala", toWrite.mkString("\n"))
    }
