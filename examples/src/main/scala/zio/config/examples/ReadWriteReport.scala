  ...
  assert(
    docs(config, Some(result)) ==
      Or(
        ...
        And(
          PathDetails(Vector("auth_token"), None, None, List("value of type string", "Prod Config")),
          PathDetails(Vector("clientid"), None, None, List("value of type string", "Prod Config"))
        )
      )
  )
}
