...
object ListSpec extends DefaultRunnableSpec {
  ...
  val spec = suite("TypesafeConfig List")(
    test("A complex enough case for testing HOCON List config and write to/back of HOCON source") {
      val configString =
        """
          |b = [
          |  {
      ...
      val zioConfigResult = TypesafeConfigSource.fromHoconString(configString) match {
        case Left(v)      => Left(v)
        case Right(value) => read(descriptor[A] from value)
      }
      ...
      val expectedReadResult = Right(expectedConfig)

      // Being able to write back HOCON
      val written = for {
        rawHocon <- write(descriptor[A], expectedConfig)
              .map(_.toHocon.render(ConfigRenderOptions.concise().setJson(true).setFormatted(true)))
        configSource <- TypesafeConfigSource.fromHoconString(rawHocon)
      } yield configSource

      val readWritten = written match {
        case Left(v)          => Left(v)
        case Right(configSource) => read(descriptor[A] from configSource)
      }

      assert(readWritten)(equalTo(zioConfigResult)) &&
      assert(zioConfigResult)(equalTo(expectedReadResult))
    },
