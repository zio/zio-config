...
object TypesafeConfigReadWriteTest
  extends BaseSpec(
    suite("read-write roundtrip tests")(
      test("Simple: read(descriptor from typesafeSource) == read(descriptor from write(descriptor, read(descriptor from typesafeSource)))") {
        val readSource =
      ...
      test("Nested: read(descriptor from typesafeSource) == read(descriptor from write(descriptor, read(descriptor from typesafeSource)))") {
        val config =
          nested("a") { string("b") }
      ...
      test("Nested List: read(descriptor from typesafeSource) == read(descriptor from write(descriptor, read(descriptor from typesafeSource)))") {
        val config =
      ...
      test("Nested Empty List: read(descriptor from typesafeSource) == read(descriptor from write(descriptor, read(descriptor from typesafeSource)))") {
        val config =
          nested("a") { list("b") { string } }
      ...
      test("Nested Config with primitive lists: read(descriptor from typesafeSource) == read(descriptor from write(descriptor, read(descriptor from typesafeSource)))") {
        val hocon =
          s"""
             | a : {
             |    b : {
        ...
        val deConfig = nested("a") {
          nested("b") {
            nested("c") {
              ((list("d"){ string } |@| list("e") { int } |@| int("f") |@| list("g") {string}))(DEF.apply, DEF.unapply)
            }
          }
        }
        ...
        val written =
          write(config, readSource.loadOrThrow).loadOrThrow.toHocon.render(ConfigRenderOptions.concise().setFormatted(true).setJson(false))
      ...
      test("Map: read(descriptor from typesafeSource) == read(descriptor from write(descriptor, read(descriptor from typesafeSource)))") {
        val readSource =
        ...
        assert((readWritten, readSource))(equalTo((readSource, Right(sss(Map("syd" -> List(1, 2), "melb" -> List(1)), Nil, List(1, 3, 3), Map("v" -> "a"))))))
      },
      test("Complex Map: read(descriptor from typesafeSource) == read(descriptor from write(descriptor, read(descriptor from typesafeSource)))") {
        val readSource =
