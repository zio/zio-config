...
trait GenerateConfig {
  ...
  def generateConfig[A: DeriveGen](
    ...
    size: Int = 0
  ): ZStream[Random with Sized with Has[Clock], String, PropertyTree[String, String]] =
    DeriveGen[A].sample
      .repeat(recurs(Math.max(0, size - 1)))
      .flatMap {
        case Some(r) => ZStream.fromZIO(ZIO.fromEither(write(config, r.value)))
  ...
  def generateConfigHoconString[A: DeriveGen](
    config: ConfigDescriptor[A],
    size: Int = 0
  ): ZStream[Random with Sized with Has[Clock], String, String] =
  ...
  def generateConfigMap[A: DeriveGen](
    ...
    size: Int,
    keyDelimiter: String = "."
  ): ZStream[Random with Sized with Has[Clock], String, Map[String, ::[String]]] =
    generateConfig(config, size).map(_.flattenString(keyDelimiter))

  implicit class UnsafeRunOps[E, A](s: ZStream[Random with Sized with Has[Clock], E, A]) {
